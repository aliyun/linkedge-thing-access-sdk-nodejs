...
const ERROR_GET_TSL_CONFIG = 'get_tsl_config';
...
const KEY_DRIVER_CONFIG = `gw_driverconfig_${FUNCTION_NAME}`;
...
class ThingAccess {
  ...
  setup() {
    if (!this._setupPromise) {
      this._setupPromise = new Promise((resolve) => {
        ...
        this._getTslConfigPromise = undefined;
  ...
  getTsl() {
    ...
    getTslConfig() {
        if (!this._getTslConfigPromise) {
            this._getTslConfigPromise = new Promise((resolve) => {
                console.info(`Get TSL config...`);
                if (!session.configInterface) {
                    throwError(ERROR_GET_TSL_CONFIG, 'Client has not been setup or setup failed.');
                }
                var getConfig = 'get_config';
                session.configInterface[getConfig](`gw_TSL_config_${this.config.productKey}`,
                    (err, code, result) => {
                        if (err) {
                            throwError(ERROR_GET_TSL_CONFIG, err);
                        }
                        if (code !== 0) {
                            throwError(ERROR_GET_TSL_CONFIG, `Get config failed: errno = ${code}`);
                        }
                        // Check if the result is JSON-welled.
                        JSON.parse(result);
                        console.info(`Get TSL config: ${result}`);
                        resolve(result);
                    });
            }).catch(err => {
                this._getTslConfigPromise = undefined;
        ...
        return this._getTslConfigPromise;
    }
          ...
          .then((ret) => {
            ...
            if (name === 'get') {
              if (!ret.code && !ret.params) {
                throw new Error(`"params" is required when "get" callback returns successful.`);
              }
...
module.exports.ERROR_GET_TSL_CONFIG = ERROR_GET_TSL_CONFIG;
