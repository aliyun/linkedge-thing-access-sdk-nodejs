...
class ThingAccess {
  ...
  register() {
    if (!this._registerPromise) {
      this._registerPromise = new Promise((resolve) => {
        ...
        return this._registerThing(this.config.productKey,
          this.config.deviceName || this.config.localName, tsl);
      }).then((thingId) => {
  ...
  unregister() {
    if (!this._unregisterPromise) {
      this._unregisterPromise = new Promise((resolve) => {
        ...
        if (!this._thingId) {
          throw new Error('Thing has not been registered or has cleanup.');
        }
  ...
  _registerThing(productKey, thingName, tsl) {
    return new Promise((resolve) => {
