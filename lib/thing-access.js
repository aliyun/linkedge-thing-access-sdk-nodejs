...
// It must be a constant functional expression defined after session here.
const getDriverConfig = () => {
  return new Promise((resolve) => {
    ...
    return new Promise((resolve) => {
      ...
      session.configInterface[getConfig](`gw_driverconfig_${FUNCTION_NAME}`,
        (err, code, result) => {
          if (err) {
            throw err;
          }
...
class ThingAccess {
      ...
      this._cleanupPromise = new Promise((resolve) => {
        ...
        if (this._setupPromise) {
          ...
          if (session.things.size === 0) {
            return session.finalize().catch(err => {
              console.warn(`Failed to finalize: ${err}.`);
            });
          }
        }
      }).then(() => {
  ...
  _exportDefaultThingInterface(serviceName) {
    ...
    var _handleConnectResult = function (result) {
      if (!result) {
        console.warn('Illegal connect result: null.');
        return null;
      }
      ...
      try {
        ...
        console.warn('Illegal connect result: not JSON.');
        return null;
      }
      ...
      if (!params) {
        console.warn('Illegal connect result: params not found');
        return null;
      }
      console.info(
...
module.exports.getDriverConfig = getDriverConfig;
